<html>
<head>
<title>MagnusDemo</title>
<link type="text/css" rel="stylesheet" href="css/stylesheet.css">
</head>
<body>
<div id="main">
<div id="header">
	<img src="images/logo.png" height="70px">
	<nav id="nav-3">
	<a class="link-3" href="index.jsp">Free Trail</a>
	<a class="link-3" href="index.jsp">Sign In</a>
	<a class="link-3" href="admin.jsp">Admin</a>
	</nav>
</div>
<div class="bod">
	<center>
	<nav id="nav-2">
	<a class="link-2" href="index.jsp">User Login</a>
	<a class="link-2" href="guest.jsp">Guest Login</a>
	</nav>
                    <form action="guestlogin.jsp">
			<table>
			<tr><td><h3>Guest Login Page</h3></td></tr>
                        <tr><td><h5>Plz enter the valid username and password</h5></td></tr>
				<tr><td><input type="text" placeholder="Username" name="Username" ><br><br>
				<tr><td><input type="password" name="Password" placeholder="Password"></td></tr>
				<tr><td><input type="submit" value="Login"></td></tr>
			</table>
                    </form>
			<label> <b>UserName : guest   Password: magnus</b></label>
		</center>
</div>
<div id="img1">
	<a href="https://www.linkedin.com/jobs/" target="_blank"><img src="images/n2.jpg" width="300px" height="200"></a><br><br><br><br>
	<a href="https://my.naukri.com/account/createaccount?othersrcp=23531&wExp=N&gclid=CPnssYXKm9MCFdGIaAod5_QI4g" target="_blank"><img src="images/n1.jpg" width="300px" height="200"></a>
</div>
<div id=footer>
<center><div>Magnus Pvt. Ltd., &copy; 2017, ALL RIGHTS RESERVED</div>
<div> <a href="" >Subscribe To News Letter</a></div>
<div class="feed"><a href="" >FeedBack</a></div></center>
</div>
</div>
</body>
</html>




