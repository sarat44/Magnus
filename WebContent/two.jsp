<html>
<head>
<title>Magnus Application</title>
<link type="text/css" rel="stylesheet" href="css/stylesheet1.css">
</head>
<body>
	<div class="main">
    	<div id="header1">
       		<a href="index.jsp"> <img src="images/logo.png" height="70px"></a>
			<nav id="nav-3">
            <a class="link-3" href="student.jsp">Students</a>
            <a class="link-3" href="studentSe.jsp">Students Search</a>
            <a class="link-3" href="user.jsp">Users</a>
            <a class="link-3" href="userSe.jsp">Users Search</a>
            <a class="link-3" href="moreDetails.jsp">More</a>
            <a class="link-3" href="index.jsp">LogOut</a>
    		</nav>
    	</div>
    	<div id="multiplet">
    			<center><h2>Multiple Tabs</h2>
    			<table>
    			<nav id="nav-2">
            <a class="link-2" href="moreDetails.jsp">One</a>
            <a class="link-2" href="two.jsp">Two</a>
            <a class="link-2" href="three.jsp">Three</a>
    		</nav>
    			</table>
			<table>
			<tr><td><input type="text" name="tab" placeholder="Tab2 TextField 1"></td></tr>
			<tr><td><input type="text" name="tab1" placeholder="Tab2 TextField 1"></td></tr>
    			</center>
    			
    				<tr>
    			</table>
    		
    	</div>
    	<br/><br/><br/>
    	<div id="sidebar">
    		 <ul>
 				 <li><a href="">Multiple Tabs</a></li>
 				 <li><a href="">Menu</a></li>
				 <li><a href="">Auto Complete</a></li>
  				 <li><a href="">Collapsible Content</a></li>
  				 <li><a href="">Slider</a></li>
  				 <li><a href="">Tooltips</a></li>
  				 <li><a href="">Popups</a></li>
  				 <li><a href="">Links</a></li>
  				 <li><a href="">CSS Properties</a></li>
  				 <li><a href="">Iframes</a></li> 
			</ul> 
    	</div>
    	
    	<div class=footer>
<center><div>Magnus Pvt. Ltd., &copy; 2017, ALL RIGHTS RESERVED</div>
<div> <a href="" >Subscribe To News Letter</a></div>
<div class="feed"><a href="" >FeedBack</a></div></center>
</div>
    	</div>
</body>
</html>